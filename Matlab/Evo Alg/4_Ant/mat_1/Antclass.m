	...
	methods
        ...
        function move(self,map,beta)
            ...
            %calculate: 
                sum=0;
                for i=1:length(prob)
                    sum=sum+map.pheromone(self.location,prob(i,1))*((1/map.link(self.location,prob(i,1))).^beta);
                end
                for i=1:length(prob)
                    prob(i,2)=map.pheromone(self.location,prob(i,1))*((1/map.link(self.location,prob(i,1))).^beta)/sum;  
                end
             %add link:   
