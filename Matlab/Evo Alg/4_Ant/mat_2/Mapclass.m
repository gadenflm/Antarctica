classdef Mapclass < handle
	...
	methods
        ...
        function update_2(self,min_path,min_len)
            delta_p=1/min_len;
            for i=1:(length(min_path)-1)
               self.pheromone(min_path(i),min_path(i+1))=self.pheromone(min_path(i),min_path(i+1))+delta_p;
               self.pheromone(min_path(i+1),min_path(i))=self.pheromone(min_path(i+1),min_path(i))+ delta_p;
            end
