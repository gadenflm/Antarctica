    ...
    public partial class Cutter : Form
    {
        ...
        private void Cutter_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left && CatchFinished)
            {
                ...
                string strname = currentTime.ToString();
                string filename = null;
                foreach (char c in strname)
                {
                    int a = (int)c;
                    if ((a > 47) && (a < 58)) { filename = filename + c; } else filename = filename + "_";
                }
                string foldername = System.Environment.GetFolderPath(System.Environment.SpecialFolder.Desktop) + "\\截图";
                System.IO.Directory.CreateDirectory(foldername);
              //  Console.WriteLine(filename);
                CatchedBmp.Save(foldername + "\\" + filename + ".jpg", System.Drawing.Imaging.ImageFormat.Jpeg);
